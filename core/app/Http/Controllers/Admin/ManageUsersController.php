...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $approvedProducts = Product::where('status',1)->where('user_id',$user->id)->whereHas('user', function ($query) {
            $query->where('status',1);
        })->count();
        return view('admin.users.detail', compact('page_title', 'user','totalDeposit','totalWithdraw','totalTransaction','approvedProducts'));
    }
    ...
    {
        $page_title = 'Featured Authors';
        $empty_message = 'No data found';
        $authors = Featured::latest()->paginate(getPaginate());

        return view('admin.users.featured', compact('page_title', 'authors', 'empty_message'));
    }
    ...
    public function makeFeatured(Request $request)
    {
        ...
        if (!$user) {
            $notify[] = ['error', 'No author found.'];
            return back()->withNotify($notify);
        }

        if ($user->status != 1) {
            $notify[] = ['error', 'This author is banned.'];
            return back()->withNotify($notify);
        }

        if (count($user->products) < 3) {
            $notify[] = ['error', 'The author must have at least three products.'];
            return back()->withNotify($notify);
        }

        if (count($user->products) >= 3) {
            $previous = Featured::latest()->first();
            if($previous){
                $previous->revoked_at = Carbon::now();
                $previous->update();
            }
            ...
            $notify[] = ['success', 'This author is featured successfully.'];
            return back()->withNotify($notify);
        }else{
