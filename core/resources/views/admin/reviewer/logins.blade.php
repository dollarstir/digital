                        ...
                        <table class="table table--light style--two">
                            ...
                            @forelse($login_logs as $log)
                                <tr>
                                    ...
                                    <td data-label="@lang('Username')"><a href="{{ route('admin.reviewers.detail', $log->reviewer_id)}}"> {{ ($log->reviewer) ? $log->reviewer->username : '' }}</a></td>
                                    <td data-label="@lang('IP')">
                                        <a href="{{route('admin.reviewers.login.ipHistory',[$log->reviewer_ip])}}">
                                            {{ $log->reviewers_ip }}
                                        </a>
                                    </td>
...
@push('breadcrumb-plugins')
    @if(request()->routeIs('admin.reviewers.login.history'))
    <form action="{{ route('admin.reviewers.login.history') }}" method="GET" class="form-inline float-sm-right bg--white">
