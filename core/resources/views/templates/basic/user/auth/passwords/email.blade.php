        ...
        <form class="account-form" method="POST" action="{{ route('user.password.email') }}">
            @csrf
                <div class="form-group">
                    <label class="text-white">@lang('E-Mail Address') <sup class="text--danger">*</sup></label>
                    <div class="input-group mb-3">
                        <span class="input-group-text"><i class="las la-at fs-4"></i></span>
                        <input type="email" name="email" autocomplete="off" placeholder="@lang('Enter your email')" class="form--control @error('email') is-invalid @enderror">
                        @error('email')
                ...
                <button type="submit" class="btn btn--base w-100 mt-3">@lang('Send Password Reset Link')</button>
            </form>
